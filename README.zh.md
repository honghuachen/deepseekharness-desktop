...
│  2. 启动官方 dsh web 服务 (127.0.0.1:<port>)         │
...
![管理第三方插件窗口：按 profile 分组列出官方/第三方插件，标注 bundle/补丁层引用位置，可勾选批量移除或检查更新](docs/screenshots/plugin-manager.png)
...
![社区插件市场窗口：按分类浏览严选 Top 10 社区插件，支持搜索、跳转开源地址与一键安装到指定 profile](docs/screenshots/plugin-market.png)
...
![Token 用量统计窗口：按模型分组视图，显示总 token 数、估算费用、缓存命中率，以及每个模型的输入/输出/缓存分项](docs/screenshots/token-usage-stats.png)
